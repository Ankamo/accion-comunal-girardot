...
import { SPREADSHEET_ID_REUNION_DIRECTIVOS, SHEET_NAME_REUNION_DIRECTIVOS } from "@/config/idSheets";
import { FaCalendarAlt, FaMapMarkerAlt, FaFileAlt, FaClipboardList, FaClock } from "react-icons/fa";

// Rango amplio
const REUNION_DIRECTIVOS_RANGE = `${SHEET_NAME_REUNION_DIRECTIVOS}!A1:H`;
...
// ...resto de imports y tipos sin cambios
...
export default function AsambleaGeneralPage() {
  ...
  const SHEET_URL = `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID_REUNION_DIRECTIVOS}/values/${REUNION_DIRECTIVOS_RANGE}?key=${GOOGLE_SHEETS_API_KEY}`;
      ...
      try {
        ...
        if (data.values && data.values.length > 1) {
          ...
          const citacionesData: Citacion[] = rows
            ...
            // ⚡ Filtrar reuniones con estado "SIN PROGRAMAR"
            .filter((c: Citacion) =>
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-6">
      <h1 className="text-4xl font-extrabold text-center text-[#19295A] dark:text-blue-200 mb-8">
        Citaciones a Reunión de Directivos y Dignatarios
      </h1>
          ...
          return (
            <div key={idx} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 flex flex-col gap-4">
              <h2 className="text-2xl font-bold text-[#19295A] dark:text-blue-300">
                {cita.finalidad || "Reunión de Directivos y Dignatarios"}
              </h2>
